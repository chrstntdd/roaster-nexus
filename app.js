...
function renderDetails(thisObjDetails) {
	...
	$('.roaster-banner').css({
		'background-image':
			'linear-gradient(to bottom,rgba(0, 0, 0, 0),rgba(0, 0, 0, 0.6)), url("' + thisObjDetails.imgUrls[0] + '")',
	});
...
function renderContact(thisObjDetails) {
	...
	thisObjDetails.tel != null
		...
		: $contact
				.find('#phone')
				.text(thisObjDetails.phone != null ? thisObjDetails.phone : 'No phone number available.');
...
function renderFeedback(thisObjDetails) {
	...
	var feedbackHtml =
		'<section id="feedback">' + '<h3 id="rating"></h3>' + '<h2>REVIEWS</h2>' + '<ul></ul>' + '</section>';
